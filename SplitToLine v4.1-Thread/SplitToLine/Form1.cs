...
namespace SplitToLine
{
    public partial class Form1 : Form
    {
        ...
        // --- Fast: 45 seconds with  20 thread
        // --- Fast: 28 seconds with 100 thread
        // Step:
        // 1. Calculate Loop Count
        // 2. Create Thread and put process result into chunk of List
        // 3. Thread Join and Write to File

        // targetThreads: start() and join()
        List<Thread> chunkThreads = new List<Thread>();
        ...
        List<ThreadStream> mmfChunkList = new List<ThreadStream>();    //Converted MMF Chunk

        private void Process_MMF()
        {
            matchFound = 0;
            FileStream inFS = new FileStream(txtSource.Text, FileMode.Open, FileAccess.Read);
            ...
            try
            {
                ...
                foreach (var item in SearchBytes)
                {
                    searchStr += string.Format(@"\x{0}", item.ToString("x2", new CultureInfo("en-us")));
                }

                string replaceStr = string.Empty;
                foreach (var item in ReplaceBytes)
                {
                    replaceStr += string.Format(@"\x{0}", item.ToString("x2", new CultureInfo("en-us")));
                }
