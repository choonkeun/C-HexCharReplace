...
namespace SplitToLine
{
    public partial class Form1 : Form
    {
        ...
        // --- 60 to 100 seconds with 100 task: slower than thread but less than 45% cpu usage
        // Step:
                ...
                using (var mmf = MemoryMappedFile.CreateFromFile(txtSource.Text, FileMode.Open, "Map1"))
                {
                    ...
                    while (t < loopCount)
                    {
                        ...
                        for (int i = 0; i < maxThreads && t < loopCount; i++)
                        {
                            ...
                            await start_thread(i);    //create task
        ...
        public async Task start_thread(int id)
        {
            int res = await Task.FromResult<int>(Do_Chunk_Process(mmfChunkList[id]));
        }
