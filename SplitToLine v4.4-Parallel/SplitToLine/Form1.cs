...
namespace SplitToLine
{
    public partial class Form1 : Form
    {
        ...
        // --- 100 seconds with 100 task: slower than thread but less than 45% cpu usage
                ...
                using (var mmf = MemoryMappedFile.CreateFromFile(txtSource.Text, FileMode.Open, "Map1"))
                {
                    ...
                    while (t < loopCount)
                    {
                        ...
                        for (int i = 0; i < maxThreads && t < loopCount; i++)
                        {
                            ...
                            //start_thread(i);    //create threads

                            filePoint += readSize;
                            t++;
                        }

                        Parallel.ForEach(mmfChunkList, id => Do_Chunk_Process(id));
